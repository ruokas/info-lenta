...
import { Activity, User, Stethoscope, Shield, Clock, ChevronRight, Hash, Lock, AlertCircle, ArrowLeft, Loader2 } from 'lucide-react';
import { getAuthMode, hasStaffPin, AuthMode } from '../lib/supabaseClient';
...
const LoginView: React.FC<LoginViewProps> = ({ doctors, nurses, onLogin }) => {
  const { loginWithPin, error: authError, isLoading, clearError, authMode } = useAuth();
  ...
  const handleStaffClick = (staff: Staff) => {
    // Check auth mode
    if (authMode === 'pin' && hasStaffPin(staff.id)) {
      // Staff has PIN - show PIN entry
      setSelectedStaff(staff);
      setShowPinModal(true);
    } else {
      // Demo mode or no PIN set - direct login
      const user: UserProfile = {
        ...staff,
        isAuthenticated: true,
        loginTime: new Date().toISOString(),
        hasPin: hasStaffPin(staff.id)
      };
      onLogin(user);
    }
  };
  ...
  const handlePinSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedStaff || pinInput.length < 4) {
      setLocalError('PIN turi būti bent 4 skaitmenys');
      return;
    }
    ...
    const success = await loginWithPin(selectedStaff.id, pinInput, userProfile);
    if (success) {
      setShowPinModal(false);
      // Navigation will be handled by App.tsx after login
      onLogin({ ...userProfile, isAuthenticated: true });
    }
  };
    ...
    <div className="flex h-screen w-full bg-slate-950 overflow-hidden font-sans text-slate-100 relative">
                    ...
                    >
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          {hasStaffPin(staff.id) && authMode === 'pin' && <Lock size={14} className="text-amber-400" />}
      ...
      {showPinModal && selectedStaff && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-8 max-w-md w-full shadow-2xl animate-in zoom-in-95 duration-300 relative">
            <button
            ...
            <div className="text-center mb-6">
              <div className={`
                w-16 h-16 mx-auto rounded-full flex items-center justify-center text-xl font-bold mb-4 text-white
              ...
              <p className="text-slate-400 text-sm">Įveskite PIN kodą</p>
            </div>

            {(authError || localError) && (
              <div className="bg-red-900/30 border border-red-500/50 rounded-lg p-3 mb-4 flex items-center gap-2">
                <AlertCircle size={16} className="text-red-400" />
                <span className="text-red-200 text-sm">{authError || localError}</span>
              </div>
            )}
            ...
            <form onSubmit={handlePinSubmit} className="space-y-4">
              <div>
                <input
                  ...
                  placeholder="••••"
                  className="w-full bg-slate-800 border border-slate-600 text-center text-3xl font-mono tracking-[0.5em] text-white rounded-xl px-4 py-4 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition"
                  autoFocus
                />
              </div>
              <button
                ...
                className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 rounded-xl shadow-lg shadow-blue-900/40 transition transform active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                {isLoading ? (
                  <><Loader2 size={18} className="animate-spin" /> Tikrinama...</>
                ) : (
                  <><Lock size={18} /> Prisijungti</>
                )}
              </button>
            </form>
            ...
            <button
              onClick={() => setShowPinModal(false)}
              className="w-full mt-3 text-slate-400 hover:text-white text-sm py-2 transition"
            >
              Grįžti atgal
            </button>
          </div>
