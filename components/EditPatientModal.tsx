...
const EditPatientModal: React.FC<EditPatientModalProps> = ({ bed, beds, doctors, nurses, currentUser, medicationBank = [], isOpen, onClose, onSave, workShifts, protocols = [], onMovePatient }) => {
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="bg-slate-950/50 p-4 rounded-xl border border-slate-800 space-y-4">
                        <div className="grid grid-cols-3 gap-3">
                          <div><label className="text-[10px] text-slate-500 uppercase block mb-1">AKS</label><div className="flex items-center gap-1"><input type="number" placeholder="120" value={formData.patient.vitals?.bpSystolic || ''} onChange={(e) => handleVitalsChange('bpSystolic', e.target.value)} className="w-full bg-slate-800 border border-slate-700 rounded p-2 text-center font-bold text-base" /><span className="text-slate-500 pb-1">/</span><input type="number" placeholder="80" value={formData.patient.vitals?.bpDiastolic || ''} onChange={(e) => handleVitalsChange('bpDiastolic', e.target.value)} className="w-full bg-slate-800 border border-slate-700 rounded p-2 text-center font-bold text-base" /></div></div>
